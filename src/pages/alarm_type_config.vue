    ...
    <div class="event event-wrapper">
        ...
        <div class="attendance-list table-area" style="padding-bottom: 10px">
            <div class="ub ub-pj w100">
                ...
                <div class="ub">
                    <addBtn icon="el-icon-plus" title="添加类型" @click="addDialog = true" />
                </div>
            </div>
            <el-table ref="multipleTable" v-loading="loading" class='bigTable full-table' :data="tableData" border stripe tooltip-effect="dark"
                ...
                <el-table-column prop="name" label="告警类型名称">
                </el-table-column>
                <el-table-column align="center" label="操作" width="120">
                    <template slot-scope="scope">
                        <el-button v-if="scope.row.is_sys === 0" type="text" @click="handleEdit(scope.row)">编 辑</el-button>
                        <el-button v-if="scope.row.is_sys === 0" type="text" @click="delUserBtn(scope.row)">删 除</el-button>
                        <el-button style="color: rgba(255, 255, 255, .5)" v-if="scope.row.is_sys === 1" type="text">内置类型</el-button>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <!--增加内容开始-->
        <el-dialog title="添加告警类型" :visible.sync="addDialog" width="790px" custom-class="attendance-dialog">
            <el-form :model="alarmTypeForm" :rules="rules" ref="alarmTypeForm" :inline="true" class="unit">
                <el-form-item label="告警类型名称：" prop="name" :label-width="formLabelWidth">
                    <el-input size="small" style="width: 300px" v-model="alarmTypeForm.name" placeholder="请输入" autocomplete="off" clearable></el-input>
                </el-form-item>
            </el-form>
        ...
        <!--编辑内容开始-->
        <el-dialog title="编辑告警类型" :visible.sync="editDialog" width="790px" custom-class="attendance-dialog">
            <el-form :model="alarmTypeFormEdit" :rules="rules" ref="alarmTypeFormEdit" :inline="true" class="unit">
                <el-form-item label="告警类型名称：" prop="name" :label-width="formLabelWidth">
                    <el-input size="small" style="width: 300px" v-model="alarmTypeFormEdit.name" placeholder="请输入" autocomplete="off" clearable></el-input>
                </el-form-item>
            </el-form>
            <span slot="footer" class="dialog-footer">
        ...
        <!--删除-->
        <el-dialog title="删除告警类型" :visible.sync="userDeleteDialog" width="30%" custom-class="attendance-dialog">
            <span>确定删除吗？</span>
            <span slot="footer" class="dialog-footer">
